        ...
        <div class="order-summary">
            ...
            <ul>
                <li v-for="item in cart" :key="item.product._id">
                    <p>{{ item.product.name }} - {{ item.product.price[currency].amount }} {{ currency === 'euros' ? '€' : '₽' }} x {{ item.quantity }}</p>
                </li>
            </ul>
            <p>{{ $t('checkoutView.total') }}: {{ totalPrice }} {{ currency === 'euros' ? '€' : '₽' }}</p>
        </div>
        ...
        <!-- <div v-if="!isAuthenticated"> -->
            ...
            <form @submit.prevent="handleCheckout">
                ...
                <div>
                    ...
                    <input v-model="recipient.surname" type="text" id="surname" required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input v-model="shippingAddress.country" type="text" id="country" required />
                </div>
                <div>
                    <label for="city">{{ $t('checkoutView.city') }}</label>
                    <input v-model="shippingAddress.city" type="text" id="city" required />
                </div>
                <div>
                    <label for="street">{{ $t('checkoutView.street') }}</label>
                    <input v-model="shippingAddress.street" type="text" id="street" required />
                </div>
                <div>
                    <label for="buildingNumber">{{ $t('checkoutView.buildingNumber') }}</label>
                    <input v-model="shippingAddress.buildingNumber" type="text" id="buildingNumber" required />
                </div>
                <div>
                    <label for="apartment">{{ $t('checkoutView.apartment') }}</label>
                    <input v-model="shippingAddress.apartment" type="text" id="apartment" />
                </div>
                <div>
                    <label for="postalCode">{{ $t('checkoutView.postalCode') }}</label>
                    <input v-model="shippingAddress.postalCode" type="text" id="postalCode" required />
                </div>
